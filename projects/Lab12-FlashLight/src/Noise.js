...
export class Noise {
    ...
    static fractalPerlinNoise(p, layerCount) {
      ...
      for (let i = 0; i < layerCount; i += 1) {
        ...
        sum += this.perlinNoise(p.scalarMultiply(1 / (1 << i))) * weight;
      }
      return sum;
    }
      ...
      for (let z = 0; z < dimensions.z; ++z) {
        for (let y = 0; y < dimensions.y; ++y) {
          for (let x = 0; x < dimensions.x; ++x) {
            const p = new Vector3(x * scale.x, y * scale.y, z * scale.z);
            const value = Noise.fractalPerlinNoise(p, layerCount) * 0.5 + 0.5;
